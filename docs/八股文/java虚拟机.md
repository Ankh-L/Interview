...
## 简述G1垃圾收集器
g1不再坚持固定大小以及固定数量的分代区域划分，而是把连续的Java堆划分为多个大小相等的独立区域（region）。每一个region可以根据需要扮演新生代的Eden区域，survivor区域，老年代区域。根据不同的角色采取不同的回收策略。
...
由于需要记录跨region的引用，需要消耗更多的内存。在小内存应用上CMS大概率仍然要优于G1，优劣势的平衡点在6~8G之间。
